...
void attack_init_sliding() {
  // TODO: Implement moves for rooks and bishops
}
...
uint64_t gen_bishop_attack(const Sq sq, uint64_t blockerBoard) {
  ...
  for (r = sr + 1, f = sf + 1; r <= 7 && f <= 7; r++, f++) {
    set_bit(output, SQ(r, f));
    if (get_bit(blockerBoard, SQ(r, f)))
      break;
  }
  // NW direction
  for (r = sr + 1, f = sf - 1; r <= 7 && f >= 0; r++, f--) {
...
uint64_t gen_rook_attack(const Sq sq, const uint64_t blockerBoard) {
  ...
  for (r = sr + 1; r <= 7; r++) {
    set_bit(output, SQ(r, sf));
    if (get_bit(blockerBoard, SQ(r, sf)))
      break;
  }
  // S direction
  for (r = sr - 1; r >= 0; r--) {
  ...
  for (f = sf + 1; f <= 7; f++) {
    set_bit(output, SQ(sr, f));
    if (get_bit(blockerBoard, SQ(sr, f)))
      break;
  }
  // W direction
  for (f = sf - 1; f >= 0; f--) {
...
uint64_t find_magics(const Sq sq, const int relevant_bits, const PieceType piece) {
    ...
    uint64_t possible_occ = (piece == BISHOP) ? gen_bishop_occupancy(sq)
                                                               : gen_rook_occupancy(sq);
...
uint64_t get_bishop_attack(const Sq sq, uint64_t blockerBoard) {
    blockerBoard &= bishop_occ_mask[sq];
    blockerBoard *= bishop_magics[sq];
    blockerBoard >>= (64 - bishop_relevant_bits[sq]);
    return bishop_attacks[sq][blockerBoard];
}

uint64_t get_rook_attack(const Sq sq, uint64_t blockerBoard) {
    blockerBoard &= rook_occ_mask[sq];
    blockerBoard *= rook_magics[sq];
    blockerBoard >>= (64 - rook_relevant_bits[sq]);
    return rook_attacks[sq][blockerBoard];
}

uint64_t get_queen_attack(const Sq sq, uint64_t blockerBoard) {
    return get_bishop_attack(sq, blockerBoard) | get_rook_attack(sq, blockerBoard);
}