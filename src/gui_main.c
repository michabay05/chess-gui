...
void draw_board(const GUI_Board gb) {
  for (int r = 0; r < 8; r++) {
    for (int f = 0; f < 8; f++) {
      ...
      if ((int)gb.selected == SQ(r, f) &&
          pos_get_piece(gb.board.pos, gb.selected) != E)
...
void draw_pieces(Texture2D *tex, Piece piece, int row, int col) {
  ...
  DrawTexturePro(*tex,
                 (Rectangle){type * frame_width, color * frame_height,
                             frame_width, frame_height},
                 (Rectangle){pos[0], pos[1], dim[0], dim[1]}, (Vector2){0, 0},
                 0, WHITE);
}
